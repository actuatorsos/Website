...
//!
//! يتطلب تفعيل ميزة `headless_chrome` وتثبيت Chrome على الخادم.
...
mod chrome {
    ...
    pub async fn screenshot_handler(
        Query(params): Query<super::ScreenshotParams>,
    ) -> impl IntoResponse {
...
/// Fallback when headless_chrome feature is not enabled
#[cfg(not(feature = "headless_chrome"))]
async fn screenshot_unavailable(
    Query(_params): Query<ScreenshotParams>,
) -> impl IntoResponse {
    (
...
pub fn routes() -> Router<crate::db::AppState> {
    #[cfg(feature = "headless_chrome")]
    { Router::new().route("/", get(chrome::screenshot_handler)) }

    #[cfg(not(feature = "headless_chrome"))]
    { Router::new().route("/", get(screenshot_unavailable)) }
}
